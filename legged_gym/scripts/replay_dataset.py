...
def play(args):
    ...
    while traj_idx < len(env.amp_loader.trajectory_lens):
        ...
        linear = env.amp_loader.get_linear_vel_batch(
            env.amp_loader.get_full_frame_at_time_batch(
                np.array([traj_idx]), np.array([t])
            )
        )

        ang = env.amp_loader.get_angular_vel_batch(
            env.amp_loader.get_full_frame_at_time_batch(
                np.array([traj_idx]), np.array([t])
            )
        )
            
        #print(env.get_amp_observations()[0, 12:15])
        #print(linear[0])
        print('------------------------------------')
        print(env.get_amp_observations()[0, 15:18])
        print(ang[0])
